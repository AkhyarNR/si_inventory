<?php
    ...
    $html = <<<EOD
    <h1>
    Laporan Data Barang Keluar SI Inventori LAB.RPL
    </h1>
...
EOD;
    ...
    $table = '<p align="left"> Periode : '.date('d M Y',strtotime($tglAw)).' s/d '.date('d M Y',strtotime($tglAk)).'</p> <table border="1"  cellpadding="0" cellspacing="3" align="center" fontsize="12">';
        $table .= '
            ...
            <th colspan="1" width="130">Asal Barang</th>
            <th colspan="1" width="110">Tanggal Masuk</th>
            </tr>
            ';
        $no=1;
        foreach($dataBahanMasuk as $keluar){
        $table .='
            <tr>
            <td width="60">'.$no.'</td>
            ...
            <td colspan="1" width="130">'.$keluar->asal_barang.'</td>
            <td colspan="1" width="110">'.$keluar->tanggal_masuk.'</td>
            </tr>';
        $no++;
        }
    ...
    $pdf->Output('Laporan_BahanMasuk.pdf', 'I'); 
?>
